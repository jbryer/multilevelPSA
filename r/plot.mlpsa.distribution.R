...
plot.mlpsa.distribution <- function(multilevelPSA, treat='Public', 
	...
	p = ggplot(level1.summary, aes_string(x=xname, y=treat))
	p = p + scale_y_continuous(limits=plot.range)
	if(flip) {
		p = p + coord_flip() 
		p = p + opts(legend.position=c(-1,-1), 
					 axis.text.x=theme_text(size=10, angle=0, hjust=.5, vjust=1))
	} else {
		p = p + opts(legend.position=c(-1,-1), 
	...
	p = p + scale_y_continuous(limits=plot.range)	
	p = p + geom_point(data=level2.summary, aes_string(x=xname, y=treat, size='n', fill=xname), 
					   stat='identity', shape=21, colour='black')
	p = p + ylab(label) + xlab(level2.label)
	p = p + geom_rug(data=level1.summary, aes_string(x=NULL, y=treat, colour=xname), alpha=.6, size=.5)
