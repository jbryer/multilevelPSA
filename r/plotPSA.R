...
plotpsa.multilevel.psa <- function(multilevelPSA,
	...
	if(labelLevel2) {
		p = p + geom_text(data=multilevelPSA$level2.summary, aes(x=level2, label=prettyNum(diffwtd, digits=3, drop0trailing=FALSE)), y=(min(multilevelPSA$level2.summary$ci.min)+1), size=3, hjust=1)
	}
